...
@Service
@ServerEndpoint(value = "/websocket/{sender}")
public class websocketservice {	
  ...
  //静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
	...
	public void onOpen(@PathParam(value = "sender") String sender,Session session){
		...
		this.sender=split[0];
		this.userid=split[1];
		System.out.println(routeTable.containsKey(sender));
		//如果用户已经连接了就不进行重复保存
	...
	public void onMessage(String message, List<UserVO> users){
		...
		for (Map.Entry<String, Object> entry : routeTable.entrySet()) {
					try {
						if(touser.length>1) {							
							//发消息给指定用户
							//System.out.println(+":"+entry.getKey()==touser[1]);
							if(entry.getKey().equals(touser[1])) {
								System.out.println("发消息给指定用户:"+touser[0]+":"+touser[1]);
								//!#!@!作为标识符前端接收到字符串后判断前面是否有这个字符，有则是单对单聊天否则是群聊
								((websocketservice) entry.getValue()).sendMessage("!#!@!"+touser[0]+"|"+sender);
							}
						}else {
							//消息群发
							if(entry.getKey()!=sender) {
								((websocketservice) entry.getValue()).sendMessage(message+"|"+sender+"&"+userid);
							}	
			...
			for (UserVO user : users) {
				...
				try {
					if(username!=null)
					//将消息发给在线的用户
					((websocketservice) routeTable.get(username)).sendMessage("toUser|"+message);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
